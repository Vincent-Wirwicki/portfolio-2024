...
float gnoise(vec2 st) {
    ...
    void main() {
        vUv = uv;
        vec3 p = position;
        float mX = smoothMod(p.x * 2. + uTime *1., 1.15, 1.5)* .1;
        float mY = smoothMod(p.y*4. + uTime , 1.75, 1.8)* .75;
        float time = sin(mY * 2. * PI) * cos(mX * 2. * PI);
        float freq = mix(0.5, 1.5, time);
        float noise = gnoise3(position * 1.75 + pow(time,5.));
        vec3 newPos = position + noise * normal *0.15;
...
`;
// float mX = smoothMod(p.x * 1.75 + uTime *1., 1.15, 1.48)* .1;
//float mY = smoothMod(p.y*4. + uTime , 1.75, 1.8)* .75;
//         float noise = gnoise3(position * 1.5 + sin(mY * 2. * PI) * cos(mX * 2. * PI)  );

// float mX = smoothMod(p.x * 2. + uTime *2., 1.75, 1.8)* .5;
// float mY = smoothMod(p.y*4. + uTime *2., 1.75, 1.8)* .5;
// float noise = gnoise3(position * 2.5 + sin(mY * 2. * PI) * cos(mX * 2. * PI)  );
